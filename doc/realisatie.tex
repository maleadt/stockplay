...
\begin{tabular}{| c p{7cm} |}
	...
	BACKEND_ADMIN & Administrator rechten voor methodes betreffende de backend. \\
	DATABASE_ADMIN & Administrator rechten voor methodes betreffende de database. \\
	SCRAPER_ADMIN & Administrator rechten voor methodes betreffende de scraper. \\
	POINTS_ADMIN & Administrator rechten voor methodes betreffende het puntenbeheer. \\
	TRANSACTION_ADMIN & Administrator rechten voor methodes betreffende het transactie-beheer. \\
	
	SECURITY_UPDATE & Toegang tot methodes die effecten updaten (toevoegen van quotes). \\
	SECURITY_CREATE & Toegang tot methodes die nieuwe effecten aanmaken. \\
	SECURITY_REMOVE & Toegang tot methodes die effecten verwijderen. \\
	SECURITY_MODIFY & Toegang tot methodes die effecten wijzigen. \\
	
	USER_REMOVE & Toegang tot methodes die gebruikers verwijderen.
...
\function{Portfolio.List}
	...
	{ array van structs met basisinformatie van de effecten:
		...
		\end{itemize_compact} }
	{ BACKEND_ADMIN, of LOGGEDIN (maar dan wordt de resultaatset gereduceerd tot effecten van de ingelogde gebruiker) }
...
\function{Order.List}
	...
	{ lijst van structs met order-informatie:
		...
		\end{itemize_compact} }
	{ BACKEND_ADMIN (moet ooit ORDER_ADMIN worden), of LOGGEDIN (maar dan wordt de resultaatset gereduceerd tot orders van de ingelogde gebruiker) }
...
\function{Order.Create}
	...
	{ BACKEND_ADMIN (moet ooit ORDER_ADMIN worden), of LOGGEDIN (indien het 'id' veld in de ingegeven struct overeenstemp met die van de ingelogde gebruiker) }
...
\function{Order.Cancel}
	...
	{ een filter, met de volgende mogelijjke sleutels:
		...
		\end{itemize_compact} }
	{ geen }
	{ BACKEND_ADMIN (moet ooit ORDER_ADMIN worden), of LOGGEDIN (maar dan wordt de set waarop de functie inwerkt gereduceerd tot orders van de ingelogde gebruiker) }

\function{Order.Modify}
	{ een bestaande order wijzigen }
	{ een filter die orders selecteert, en een struct met de te-wijzigen orderinformatie:
		\begin{itemize_compact}
		\item{}
		\end{itemize_compact} }
	{ geen }
...
\function{Transaction.List}
	...
	{ array van structs die transactie-informatie bevatten:
		...
		\end{itemize_compact} }
	{ TRANSACTION_ADMIN, of LOGGEDIN (maar dan wordt de set waarop de functie inwerkt gereduceerd tot orders van de ingelogde gebruiker) }
...
\function{Transaction.Modify}
	{ nieuwe transactie aanmaken }
	{ een filter die transacties selecteert, en een struct met de te-wijzigen transactieinformatie:
		\begin{itemize_compact}
		\item{}
		\end{itemize_compact} }
	{ id van de aangemaakte transactie }
...
\function{Points.List}
	...
	{ array van structs die transactiepunt-informatie bevatten:
		...
		\item{...}
		\end{itemize_compact} }
	{ POINTS_ADMIN, of LOGGEDIN (maar dan wordt de set waarop de functie inwerkt gereduceerd tot orders van de ingelogde gebruiker) }
...
\function{Points.CreateTransactionify}
	{ nieuwe transactie aanmaken }
	{ een filter die transacties selecteert, en een struct met de te-wijzigen transactieinformatie:
		\begin{itemize_compact}
		\item{}
